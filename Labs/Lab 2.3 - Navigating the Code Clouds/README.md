...
> [!NOTE]
> Note that the `UpdateFlightStatus` method has a high code complexity, calculated by the [Cyclomatic Complexity metric](https://en.wikipedia.org/wiki/Cyclomatic_complexity). This is a good candidate for refactoring.
...
> [!NOTE]
> We take Copilot to the edge of its capabilities. Retry the prompt provided later in the lab if you are not successful the first time.
